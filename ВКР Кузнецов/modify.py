...
def patch_nextentries():
    ...
    for i, lastpos in entries:
        nextpos = lastpos + 1
        if nextpos < len(data[i]):
            item = data[i][nextpos]
            index[item].append((i, nextpos))
...
def main():
    """Основная функция"""
    print("Применение модификации для PrefixSpan...")
    ...
    if nextentries_patched:
        print("Модификация успешно применена!")
        return 0
    else:
        print("Не удалось применить модификацию!")
        return 1
